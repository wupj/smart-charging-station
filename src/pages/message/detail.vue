    ...
    <scroll-view scroll-y class="message-content">
      <view class="message-card">
        ...
        <view class="message-type">
          <uni-icons :type="getMessageIcon(message.type)" size="48" :color="getMessageColor(message.type)"></uni-icons>
        </view>
      ...
      <view v-if="relatedMessages.length > 0" class="related-section">
        ...
        >
          <view class="related-icon">
            <uni-icons type="chatbubble-filled" size="24" color="#95a5a6"></uni-icons>
          </view>
...
const messages = ref([
  {
    ...
    unread: true,
    detail: '您的充电订单已成功支付。订单号：20230101123456789。充电桩：1号桩。金额：15.00元。感谢您的使用！如有疑问，请联系客服：400-888-8888。',
    actions: [
  ...
  },
  {
    ...
    unread: false,
    detail: '您的充电订单已成功支付。订单号：20230101123456790。充电桩：2号桩。金额：12.50元。感谢您的使用！如有疑问，请联系客服：400-888-8888。',
    actions: [
      { id: 1, text: '查看订单', type: 'view_order' }
    ]
  },
  {
    ...
    unread: false,
    detail: '尊敬的用户，美华智充元科创新园充电站将于2023年10月26日进行设备维护，届时将暂停营业。维护期间给您带来的不便，敬请谅解。预计恢复时间：2023年10月27日08:00。如有紧急充电需求，请前往附近的其他充电站。',
    actions: [
      { id: 1, text: '查看附近充电站', type: 'nearby_stations' }
    ]
  }
])

const message = computed(() => {
  return messages.value.find(m => m.id === messageId.value) || messages.value[0]
})

const relatedMessages = computed(() => {
  return messages.value.filter(m => m.id !== messageId.value && m.type === message.value.type).slice(0, 2)
})
